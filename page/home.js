...
function getMatomoSiteId() {
  const query = `query getMatomoSiteId{ getMatomoSiteId }`;
  postQuery('getMatomoSiteId', query);
}

function maxDiscountMembershipPlan() {
  const query = `query maxDiscountMembershipPlan {
  ...
  postQuery('maxDiscountMembershipPlan', query);
}

function settings() {
  const query = `query settings($keyNames:[SettingKey]) {
        settings(keyNames: $keyNames) {
            key
            value
        }
    }`;
  postQuery('settings', query, {
    keyNames: [
...
function activeChannel() {
  const query = `query activeChannel {
        activeChannel {
            id
            createdAt
            updatedAt
  ...
  postQuery('activeChannel', query);
}

function getSubscribeMessageTemplateId() {
  const query = `query getSubscribeMessageTemplateId {
        getSubscribeMessageTemplateId {
            couponGrants
        }
    }`;
  postQuery('getSubscribeMessageTemplateId', query);
}

function userClick() {
  const mutation = `mutation userClick{
        userClick
    }`
  postQuery('userClick', mutation);
}
