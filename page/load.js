...
export default function () {
  getMatomoSiteId(failureRate);
  maxDiscountMembershipPlan(failureRate);
  reportedDistributorRecord(failureRate);
  activeCustomer(failureRate);
  getDistributor(failureRate);
  getUserMember(failureRate);
  settings(failureRate);
  activeChannel(failureRate);
  getActiveOrderByType(failureRate);
  getSubscribeMessageTemplateId(failureRate);
  userClick(failureRate);
}
